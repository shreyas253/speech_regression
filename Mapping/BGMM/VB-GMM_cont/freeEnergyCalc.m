...
elseif strcmp(op.Pi_Type,'DD')
    ...
    term3 = sum((prior.alpha -1).*extra.E_lnPik) - log_cAlpha - sum((post.DDalpha -1).*(extra.E_lnPik));
end
...
if strcmp(op.cov_Type,'Full')    
    ...
    for k=1:K
        invW0 = inv(prior.W0(:,:,k));
        mkMinusm0 = post.m(:,k) - prior.m0(:,k);%D*1
        term4_1_all(k) = - D*prior.beta0(k)/post.beta(k) ...
            - prior.beta0(k)*post.v(k)*(mkMinusm0'*post.W(:,:,k)*mkMinusm0) ...
            + (prior.v0(k)-D-1)*extra.E_lnPresk(k) ...
