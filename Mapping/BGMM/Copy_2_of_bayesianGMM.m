function  [xPred,xPred_durConv,model] = bayesianGMM( X,x,x_durConv,ids,K,noLoops )
...
%   poterior predictive of VB-GMM with conjugate NIW prior is multivariate-T mixture
    ...
    % x_durConv
    xPred_durConv = cell(length(x_durConv),noLoops);
    if ~isempty(x_durConv)
        dPred = size(x_durConv{1},2);
        for itr= 1:noLoops
            for i = 1:length(x_durConv)
                if itr==1
                    xPred_durConv{i,itr} = x_durConv{i};
                else
                    xPred_durConv{i,itr} = xPred_durConv{i,itr-1};
                end
                tmp = xPred_durConv{i,itr}(ids{i},:);
                ...
                xPred_durConv{i,itr}(ids{i},:) = (tmp .* repmat(sig(dPred+1:end),size(tmp,1),1)) + repmat(m(dPred+1:end),size(tmp,1),1);
            end
        end
    end
    % x
    ...
    tmp = (tmp - repmat(m(1:dPred),size(tmp,1),1)) ./ repmat(sig(1:dPred),size(tmp,1),1);
    [tmp,pos_wei1] = multTmm_prediction(tmp,postPred); % function very similr to gmm_prediction.m except with mult-T distinution
    xPred = (tmp .* repmat(sig(dPred+1:end),size(tmp,1),1)) + repmat(m(dPred+1:end),size(tmp,1),1);
end
