function mapping_main( feats_from,feats_to,dtw_ids,allFeats,config,METADATA,propMain,currPath,featPath,silences )
...
for s = 1:length(uniqueProperty)
    ...
    if strcmp(config.normOpt,'spDep')
        ...
        for ss = 1:length(currProps)
            ...
            mMain = mean(X_curr(ids_tmp,:));
            stdMain = std(X_curr(ids_tmp,:));
            X_curr(ids_tmp,:) = (X_curr(ids_tmp,:)-repmat(mMain,sum(ids_tmp),1))./repmat(stdMain,sum(ids_tmp),1);
            Y_curr(ids_tmp,:) = (Y_curr(ids_tmp,:)-repmat(mMain,sum(ids_tmp),1))./repmat(stdMain,sum(ids_tmp),1);
        end
        mTest = mean(x_curr);
        stdTest = std(x_curr);
    ...
    % calling mapping
    [t_curr,t_map_curr] = mapCall(X_curr,Y_curr,x_curr,x_map_curr,ids_map_curr,config.method,config.methodOpts,currPath,featPath,uniqueProperty{s});
